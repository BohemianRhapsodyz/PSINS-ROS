...
public:
	...
	friend std::ofstream & operator<<(std::ofstream &out, CVect3 &A);
};
...
public:
	...
	friend CMat3 foam(const CMat3 &B, int iter);			// Fast Optimal Attitude Matrix(FOAM)
	friend std::ofstream & operator<<(std::ofstream &out, CMat3 &A);
};
...
public:
    ...
    friend std::ofstream & operator<<(std::ofstream &out, CVect &A);
};
...
public:
    ...
    friend std::ofstream & operator<<(std::ofstream &out, CMat &A);
...
public:
    ...
    virtual int Update(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts);	// KF Time&Meas Update
...
public:
    ...
    virtual int TDUpdate(const CVect3 *pwm, const CVect3 *pvm, int nSamples, double ts, int nStep = 1);  // Time-Distributed Update
};
